			  ...
			  uint32_t version)
{
	if (!strcmp(interface, "zwlr_virtual_pointer_manager_v1")) {
		...
		mgr = wl_registry_bind(
		    registry, name, &zwlr_virtual_pointer_manager_v1_interface,
		    1);
		wl.ptr = zwlr_virtual_pointer_manager_v1_create_virtual_pointer(
		    mgr, NULL);
	}
...
void way_init()
{
	...
	wl_registry_add_listener(wl_display_get_registry(wl.dpy),
				 &registry_listener, NULL);

	wl_display_dispatch(wl.dpy);

	if (!wl.ptr) {
		fprintf(stderr, "Could not create virtual pointer (virtual "
				"pointer unsupported?)\n");
		exit(-1);
	}
	...
	if (!wl.xdg_output_manager) {
		fprintf(stderr, "Could not get xdg_output_manager object "
				"(unsupported protocol?)\n");
		exit(-1);
	}

	if (!wl.layer_shell) {
		fprintf(stderr, "Could not get layer_shell object (unsupported "
				"protocol?)\n");
		exit(-1);
	}
