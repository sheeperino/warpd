...
static void xdg_output_handle_logical_size(
    void *data, struct zxdg_output_v1 *zxdg_output_v1, int32_t w, int32_t h)
{
	struct screen *scr = data;
...
static void handle_pointer_enter(void *data, struct wl_pointer *wl_pointer,
				 ...
				 wl_fixed_t wlx, wl_fixed_t wly)
{
	...
	if (!ptr.scr) {
		...
		for (i = 0; i < nr_screens; i++) {
			...
			if (scr->overlay &&
			    surface == surface_get_wl_surface(scr->overlay))
				ptr.scr = scr;
		}
	}
}
 ...
 * creating a full screen surface for each screen in order to capture the
 * initial cursor position. I couldn't find a better way to achieve this :/.
 */
static void discover_pointer_location()
{
	size_t i;

	wl_pointer_add_listener(wl_seat_get_pointer(wl.seat),
				&wl_pointer_listener, NULL);
	...
	while (!ptr.scr) {
		 ...
		 * some reason.
		 */
		zwlr_virtual_pointer_v1_motion(wl.ptr, 0, wl_fixed_from_int(1),
					       wl_fixed_from_int(1));
...
void way_screen_draw_box(struct screen *scr, int x, int y, int w, int h,
			 const char *color)
{
	uint8_t r, g, b, a;
	...
	cairo_set_source_rgba(scr->cr, r / 255.0, g / 255.0, b / 255.0,
			      a / 255.0);
...
static void init_screen_pool(struct screen *scr)
{
	...
	scr->stride =
	    cairo_format_stride_for_width(CAIRO_FORMAT_ARGB32, scr->w);
	...
	cairo_surface = cairo_image_surface_create_for_data(
	    buf, CAIRO_FORMAT_ARGB32, scr->w, scr->h, scr->stride);
...
void init_screen()
{
	...
	for (i = 0; i < nr_screens; i++) {
		...
		scr->xdg_output = zxdg_output_manager_v1_get_xdg_output(
		    wl.xdg_output_manager, scr->wl_output);
